...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueUltraviolet_index != cur->valueUltraviolet_index)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueUltraviolet_index Changed\n");
            ret = 1;
        }
    }
    if(last->valuetemp_value != cur->valuetemp_value)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuetemp_value Changed\n");
            ret = 1;
        }
    }
    if(last->valuehumidity_value != cur->valuehumidity_value)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuehumidity_value Changed\n");
            ret = 1;
        }
    }
    ...
    if(last->valueair_pressure != cur->valueair_pressure)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueair_pressure Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueUltraviolet_index = gizY2X(Ultraviolet_index_RATIO,  Ultraviolet_index_ADDITION, dataPoints->valueUltraviolet_index); 
    devStatusPtr->valuetemp_value = gizY2X(temp_value_RATIO,  temp_value_ADDITION, dataPoints->valuetemp_value); 
    devStatusPtr->valuehumidity_value = gizY2X(humidity_value_RATIO,  humidity_value_ADDITION, dataPoints->valuehumidity_value); 

    devStatusPtr->valuelight_intensity = exchangeBytes(gizY2X(light_intensity_RATIO,  light_intensity_ADDITION, dataPoints->valuelight_intensity)); 

    devStatusPtr->valueair_pressure = exchangeWord(gizY2X(air_pressure_RATIO,  air_pressure_ADDITION, dataPoints->valueair_pressure)); 
